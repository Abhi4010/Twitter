<?php 
	...
	echo $this->Session->flash('following');
?>
<?php
	echo $this->element("c9"); 
?>
 <div style = "padding-left: 20px" class="jumbotron">
 	<h2> <?php echo $userId ?> is following <?php echo $followee_size ?> people</h2>
	<legend><?php echo __('Followings'); ?></legend>
	<?php 	
		if($follows != 'empty')
		{
			for( $i = 0; $i< sizeof($follows); $i++)
			{
				...
				echo $this->HTML->link($follows[$i]['Follow']['followee_id'],
				array('controller' => 'tweets','action' =>'profile',
					$follows[$i]['Follow']['followee_id']));
				echo "<br/> ".$userName[$i];
				if($tweetData[$i] != null)
				{
					echo "<br/>";
				 ...
				 echo $this->Form->postLink(
		 					  $this->Html->tag('i', ' Unfollow', array('class' => 'glyphicon glyphicon-remove')),
		        array('action' => 'following',$userId,'unfollow'=> $follows[$i]['Follow']['followee_id']),
		        array('escape'=>false),
		   				 __('Are you sure you want to unfollow %s?',$follows[$i]['Follow']['followee_id']),
